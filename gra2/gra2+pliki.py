...
# --- FUNKCJA OBSŁUGUJĄCA ŚCIEŻKI DLA PYINSTALLERA ---
def resource_path(relative_path):
    """Zwraca poprawną ścieżkę do plików (zarówno dla skryptu, jak i dla PyInstallera)"""
    try:
...
ekran = pygame.display.set_mode(
    (800, 600), pygame.DOUBLEBUF | pygame.HWSURFACE, vsync=1
)

# Gracz (zmiana z Nicola.png na julka.png)
grafika_sprita = pygame.image.load(resource_path("julka.png")).convert_alpha()
grafika_lewo = pygame.transform.flip(grafika_sprita, True, False)
aktualna_grafika = grafika_sprita

# Przeciwnik (Pająk)
grafika_pajaka = pygame.image.load(resource_path("spider.png")).convert_alpha()
...
try:
    dzwiek_skoku = pygame.mixer.Sound(resource_path("skok.wav"))
except Exception:
    print(
        "Ostrzeżenie: Nie znaleziono pliku skok.wav. Gra uruchomi się bez dźwięku skoku."
    )

try:
    dzwiek_kolizji = pygame.mixer.Sound(resource_path("kolizja.wav"))
except Exception:
    print(
        "Ostrzeżenie: Nie znaleziono pliku kolizja.wav. Gra uruchomi się bez dźwięku kolizji."
    )
...
while True:
    ...
    kolizja_teraz = (
        maska_gracza.overlap(maska_pajaka, (offset_x, offset_y)) is not None
    )
    ...
    if jest_kolizja:
        tekst_kolizji = czcionka.render(
            "KOLIZJA!", True, (220, 20, 60)
        )  # Kolor szkarłatny/czerwony
